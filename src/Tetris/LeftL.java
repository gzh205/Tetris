 ...
 * 1 0 0
 * */
public class LeftL extends Square{
	public LeftL(Maps m,int color) {
		this.mp=m;
		xPos=5;
		yPos=2;
		status = 0;
	...
	public boolean Right() {
		...
		else if(status==2&&xPos<mp.width-1) {
			if(mp.map[xPos+1][yPos]==0&&mp.map[xPos+1][yPos-1]==0&&mp.map[xPos+1][yPos+1]==0) {
				mp.map[xPos+1][yPos]=color;
				mp.map[xPos+1][yPos-1]=color;
				mp.map[xPos+1][yPos+1]=color;
	...
	public boolean Spin() {
		...
		if(status==0&&xPos>0) {
			...
			if(mp.map[xPos-1][yPos-1]==0&&mp.map[xPos][yPos-1]==0&&mp.map[xPos][yPos+1]==0) {
				mp.map[xPos-1][yPos-1]=color;
				mp.map[xPos][yPos-1]=color;
				mp.map[xPos][yPos+1]=color;
				mp.map[xPos-1][yPos]=0;
